document.addEventListener('DOMContentLoaded', function () {
    ...
    function exibirDados() {
        ...
        const [ano, mes, dia] = usuarioLogado.dataNascimento ? usuarioLogado.dataNascimento.split('-') : ["", "", ""];
        const dataFormatada = dia ? `${dia}/${mes}/${ano}` : "Não informado";

        document.getElementById('profile-nome').textContent = usuarioLogado.nome;
        document.getElementById('profile-cidade').textContent = usuarioLogado.cidade;
        document.getElementById('profile-email').textContent = usuarioLogado.email;
        document.getElementById('profile-telefone').textContent = usuarioLogado.telefone;
        document.getElementById('profile-dataNascimento').textContent = dataFormatada;
        document.getElementById('profile-genero').textContent = usuarioLogado.genero;
    ...
    function modoEdicao() {
        profileContainer.innerHTML = `
            <div class="profile-header">
                <div class="profile-picture" style="background-image: url('${usuarioLogado.foto}')"></div>
            ...
            <div class="profile-details-edit">
                <label>Nome:</label>
                <input type="text" id="edit-nome" value="${usuarioLogado.nome}" required />
                
                <label>Data de Nascimento:</label>
                <input type="date" id="edit-dataNascimento" value="${usuarioLogado.dataNascimento}" required />

                <label>Telefone:</label>
                <input type="text" id="edit-telefone" value="${usuarioLogado.telefone}" required />

                <label>Cidade:</label>
                <input type="text" id="edit-cidade" value="${usuarioLogado.cidade}" required />
                
                <label>Gênero:</label>
                <select id="edit-genero" required>
                    <option value="Masculino" ${usuarioLogado.genero === 'Masculino' ? 'selected' : ''}>Masculino</option>
                    <option value="Feminino" ${usuarioLogado.genero === 'Feminino' ? 'selected' : ''}>Feminino</option>
                    <option value="Prefiro não informar" ${usuarioLogado.genero === 'Prefiro não informar' ? 'selected' : ''}>Prefiro não informar</option>
                </select>

                <label>Email (não pode ser alterado):</label>
                <input type="email" id="edit-email" value="${usuarioLogado.email}" disabled />
            </div>
        ...
        `;
        document.querySelector('.save-button').addEventListener('click', salvarAlteracoes);
    ...
    logoutButton.className = 'back-button';
    logoutButton.textContent = 'Sair';
    logoutButton.style.backgroundColor = '#e53935'; // Cor vermelha
